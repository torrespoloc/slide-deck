...
interface RelevanceItem { numeral: string; period: string; role: string; }
const defaults = [{ numeral: '01', period: '2025 → Now', role: 'First design hire' }, { numeral: '02', period: '2023–2024', role: 'Freelance, four domains' }, { numeral: '03', period: 'April 2026 (3 weeks)', role: 'Shipped SideNook solo' }];

export function WhyTightrope({ items = defaults, profileImageUrl }: { items?: RelevanceItem[]; profileImageUrl?: string }) {
  return (
        ...
        <div style={{ position: 'absolute', left: 0, top: 0, width: 639, height: 966, border: '4px solid #880000', borderRadius: radius.card, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center', padding: '44px 44px 28px 44px', boxSizing: 'border-box' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 28, alignSelf: 'flex-start' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              ...
              <span style={{ fontFamily: fonts.body, fontWeight: 500, fontSize: fontSize.medium, letterSpacing: letterSpacing.wide, color: '#242424' }}>About me</span>
            </div>
            <h2 style={{ margin: 0, fontFamily: fonts.display, fontWeight: 600, fontSize: fontSize.hero, lineHeight: '100%', letterSpacing: letterSpacing.tight, color: '#242424', whiteSpace: 'pre-line' }}>{'3 things relevant \nto you'}</h2>
          </div>
